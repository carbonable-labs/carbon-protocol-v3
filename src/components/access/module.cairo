...
mod Access {
    ...
    use carbon_v3::components::access::interface::{IMinter, ICertifier, IWithdrawer};
    ...
    impl MinterImpl of IMinter<ContractState> {
        fn get_minters(self: @ContractState) -> Span<ContractAddress> {
            let role = self._hash(MINTER_ROLE);
    ...
    impl CertifierImpl of ICertifier<ContractState> {
        fn get_certifier(self: @ContractState) -> ContractAddress {
            let role = self._hash(CERTIFIER_ROLE);
    ...
    impl InternalImpl of InternalTrait {
        ...
        fn assert_only_certifier(self: @ContractState) {
            let role = self._hash(CERTIFIER_ROLE);
            let unsafe_state = AccessControl::unsafe_new_contract_state();
            AccessControl::InternalImpl::assert_only_role(@unsafe_state, role);
        }
        ...
        fn _hash(self: @ContractState, role: felt252) -> felt252 {
...
mod Test {
    ...
    fn test_minter() {
        ...
        let minters = Access::MinterImpl::get_minters(@state);
        assert(minters == array![].span(), 'Wrong minters');
        // [Assert] Add minters
        Access::MinterImpl::add_minter(ref state, CERTIFIER());
        Access::MinterImpl::add_minter(ref state, ANYONE());
        Access::MinterImpl::add_minter(ref state, MINTER());
        let minters = Access::MinterImpl::get_minters(@state);
        assert(minters == array![CERTIFIER(), ANYONE(), MINTER()].span(), 'Wrong minters');
        // [Assert] Remove 2nd minter
        Access::MinterImpl::revoke_minter(ref state, ANYONE());
        let minters = Access::MinterImpl::get_minters(@state);
        assert(minters == array![CERTIFIER(), MINTER()].span(), 'Wrong minters');
        // [Assert] Remove 1st minter
        Access::MinterImpl::revoke_minter(ref state, CERTIFIER());
        let minters = Access::MinterImpl::get_minters(@state);
    ...
    fn test_assert_minter() {
        ...
        Access::InternalImpl::assert_only_minter(@state);
    }
    ...
    fn test_certifier() {
        ...
        let certifier = Access::CertifierImpl::get_certifier(@state);
        assert(certifier == ZERO(), 'Wrong certifier');
        // [Assert] Certifier is set correctly
        Access::CertifierImpl::set_certifier(ref state, CERTIFIER());
        let certifier = Access::CertifierImpl::get_certifier(@state);
        assert(certifier == CERTIFIER(), 'Wrong certifier');
        // [Assert] Certifier is changed correctly
        Access::CertifierImpl::set_certifier(ref state, ZERO());
        let certifier = Access::CertifierImpl::get_certifier(@state);
        assert(certifier != CERTIFIER(), 'Wrong certifier');
        assert(certifier == ZERO(), 'Wrong certifier');
    }
    ...
    fn test_assert_certifier() {
        ...
        Access::InternalImpl::assert_only_certifier(@state);
    }
